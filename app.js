...
const HISTORY_FILE = path.join(__dirname, "history.json"); // 過去に見つかった事業者の台帳(削除しない限り消えない)
const NEW_FILE = path.join(__dirname, "new_domains.csv"); // 今回の実行で新しく見つかった事業者だけのリスト
...
// CSVファイルを画面表示用に読み込む(件数が多い場合は先頭からMAX_TABLE_ROWS件までに切り詰める)
...
async function runCheck(resume) {
  ...
  // 新規リストも同じタイミングで新規/追記を切り替える
  const newStream = fs.createWriteStream(NEW_FILE, {
    flags: isFreshFile ? "w" : "a",
  });
  if (isFreshFile) {
    newStream.write(CSV_HEADERS.map((h) => CSV_HEADER_LABELS[h]).join(",") + "\n");
  }
        ...
        limit(async () => {
          ...
          if (result) {
            ...
            if (result.score > 0) {
              ...
              // 台帳(history)に無いドメイン = 新規発見。new_domains.csvに書き、台帳へ追加する
              if (!history[domain]) {
                history[domain] = {
                  firstSeenAt: new Date().toISOString(),
                  score: result.score,
                };
                newStream.write(rowToCsvLine(result));
                state.newFound++;
              } else {
                // 既知のドメインはスコアだけ最新化しておく
                history[domain].score = result.score;
              }
            } else {
...
// 新規発見リストCSVダウンロード(今回の実行で新しく見つかったものだけ)
app.get("/api/download-new", (req, res) => {
  if (!fs.existsSync(NEW_FILE)) {
    return res.status(404).json({ error: "new_domains.csv がまだありません" });
  }
  res.download(NEW_FILE, "新規事業者リスト.csv");
});

// 新規発見リストを画面表示用にJSONで返す(架電済みステータスも合わせて返す)
app.get("/api/new-list", (req, res) => {
  const { headers, rows, totalRows, truncated } = loadCsvRows(NEW_FILE, MAX_TABLE_ROWS);
  const callStatus = loadCallStatus();
  const domainKey = headers[0]; // 先頭列は必ず「ドメイン」
  rows.forEach((row) => {
